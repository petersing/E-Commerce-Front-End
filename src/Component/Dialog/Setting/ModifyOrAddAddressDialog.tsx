...
mutation CreateOrUpdateAddress($ReceiverName: String!, $Address: String!, $City: String!, $Country: String!, $Phone: String!, $id: Int, $Default: Boolean) {
    CreateOrUpdateAddress(ReceiverName: $ReceiverName, Address: $Address, City: $City, Country: $Country, Phone: $Phone, id: $id, Default: $Default) {
        status
    }
}
`
...
const ModifyOrAddAddressDialog = (props: {open: boolean,  CloseDialog: Function, refetch: Function, Data?: Address_Object}) => {
    ...
    function AddAddressFunction(){
        CreateOrUpdateAddressFunction({variables: { id: props.Data?.id, ReceiverName: ReceiverName, 
    ...
    return (
        <Dialog open={props.open || Boolean(props.Data)} onClose={() => props.CloseDialog()} maxWidth='sm' fullWidth>
            ...
            <DialogActions>
                ...
                <Button variant='contained' onClick={() => {AddAddressFunction()}}>{Boolean(props.Data) ? t("Profile.change") : t("Profile.Add")}</Button>
            </DialogActions>
        </Dialog>
    )
}
