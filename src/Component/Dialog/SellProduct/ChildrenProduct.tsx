import { TextField, InputAdornment, Button, Dialog, DialogContent, DialogTitle, Grid, Table, TableHead, TableBody, TableCell, TableContainer, Paper, Typography, TableRow, Alert} from "@mui/material"
...
const ChildrenProduct = (props: {Product: any[], setDisplay: Function, Display: boolean, subkey? : number, setsubkey : Function, RemoveList?: number[]}) => {
    ...
    const [AlertContent, setAlertContent] = useState<string|null>('')
    ...
    function Add(price: number|undefined, name: string|undefined, properties: {[keys: string]: string}){
        ...
        }else if(price && name && Object.keys(properties).length > 0){
            props.Product.push({"Name": name, "Price": price, 'Properties': properties, "Quantity": Quantity})
        }else{
            setAlertContent(t("SellProduct.SubProductAlertContent"))
            return
        }
