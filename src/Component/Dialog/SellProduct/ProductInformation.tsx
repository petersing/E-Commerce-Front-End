...
const ProductInformation = (props: {setSuccessResponse: Function, ImageList: File[], ItemRender: Array<string>, SuccessResponse?: {FirstImage: string, Product_Name: string}, setAlertInformation: Dispatch<SetStateAction<string | null | undefined>>}) => {
    ...
    const [DescriptionVideoList, setDescriptionVideoList] = useState<string[]>([])
    ...
    function Publish_Product(){
      if (MainName.length > 30 || MainName.length < 2){
        props.setAlertInformation(t("SellProduct.ProductNameError"))
      }else if (Shipping.length > 50 || Shipping.length < 2){
        props.setAlertInformation(t("SellProduct.ShippingLocationError"))
      }else if (Categories.length === 0){
        props.setAlertInformation(t("SellProduct.CategoriesError"))
      }else if (ProductType.length === 0){
        props.setAlertInformation(t("SellProduct.ProductTypeError"))
      }else if (AboutProduct.length === 0){
        props.setAlertInformation(t("SellProduct.AboutProductError"))
      }else if (Children_Product.length === 0){
        props.setAlertInformation(t("SellProduct.ChildrenProductError"))
      }else{
        Product_API.CreateProduct({Description: AboutProduct, ProductName: MainName, ShippingLocation: Shipping, 
                                  ...
                                  Images: props.ImageList, DescriptionImages: DescriptionImageList, DescriptionVideos: DescriptionVideoList}).then(res => {
    ...
    return (
      <>
        ...
        <DescriptionVideoDialog open={OpenDescriptionVideo} onclose={setOpenDescriptionVideo} forceUpdate={forceUpdate} VideoList={DescriptionVideoList} setVideoList ={setDescriptionVideoList}/>
