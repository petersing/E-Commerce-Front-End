...
import { Box, Typography, Button, Grid, List, ListItem, Divider, Pagination, ListItemText, Chip, Tabs, Tab} from '@mui/material'
...
const OrderItem = (props: {OrderItem: Seller_Order_List_Object, setChatBuyer: Function, AddTransportCodeKey: React.Dispatch<React.SetStateAction<{CodeKey: string|number; Change: Boolean;} | null>>, setOpenReturnPayment: Function}) =>{
                ...
                <Grid container spacing={5} columns={15}>
                    <Grid item xs={15} sx={{display: 'flex', justifyContent: 'space-between'}}>
                        ...
                        <div>
                            ...
                            <Button variant='contained' color="success" sx={{ml: '10px'}} onClick={() => props.AddTransportCodeKey({CodeKey: props.OrderItem.id, Change: props.OrderItem.OrderProcess >= 3})}>
                    ...
                    <Grid item xs={4} sx={{display: 'flex', justifyContent: 'center', flexDirection: 'column'}}>
                        ...
                        <Chip label={`${t("Business.OrderStatus")} : ${props.OrderItem.Status === 'cancel'? t("Business.Cancel"): props.OrderItem.OrderProcess === 4 ? t("Business.Finish"): t("Business.StillProcess")}`} sx={{mb: '10px'}} 
                                    color={props.OrderItem.Status === 'cancel'? "error" : props.OrderItem.OrderProcess === 4 ? "primary": "error"}/>
...
const OrderManagementPage = () => {
    ...
    const [AddTransportCodeKey, setAddTransportCodeKey] = useState<{CodeKey: string|number, Change: Boolean}|null>(null);
