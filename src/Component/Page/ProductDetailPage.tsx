    ...
    return(
        <Grid item xs={9} id='Center Product Description'> 
            ...
            <div style={{marginBottom: '10px', display: 'flex', flexDirection: 'row'}}>
                ...
                <Typography variant='h6' sx={{ml: '5px', color: "rgb(200, 120, 175)"}} >{props.ProductDetail.SubItem[props.SubProductSelect].Name}</Typography>          
            </div>
...
const DescriptionImagesAndVideos = (props: {DescriptionImages: string[], DescriptionVideos: string[]}) => {
    const [width, setWidth] = useState(window.screen.width*0.64)

    useEffect(() => {
        function handleResize() {

            if (window.innerWidth > 1200){
                setWidth(Math.min(window.innerWidth, window.screen.width*0.64))
            }else{
                setWidth(1200)
            }
        ...
        <>
            ...
            :(Finish && ProductDetail)? 
                <div style={{width: '80%', marginLeft: 'auto', marginRight: 'auto', minWidth: '1200px', marginTop: '150px'}} id="Product Detail">
                    ...
                    <DescriptionImagesAndVideos DescriptionImages={ProductDetail.DescriptionImages} DescriptionVideos={ProductDetail.DescriptionVideos}/>
                    <Divider/>
