...
class Product_API{
    static CreateProduct(data: Publish_Product_Object){
        const {Access_Token, SubItem, Images, DescriptionImages, DescriptionVideos,  ...rest} = data
        return axios.post(Server_URL + 'api/Product/Create_Product', {...rest, DescriptionVideos: JSON.stringify(DescriptionVideos), Images: Images, DescriptionImages:DescriptionImages, SubItem: JSON.stringify(SubItem)}, {headers: {'Content-Type': 'multipart/form-data', 'Authorization': 'Bearer ' + Access_Token}})
    }
